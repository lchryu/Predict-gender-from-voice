...
def Predict_from_user_data(main_pca, mainModel):
    option = input(
        "Bạn có muốn dự đoán giới tính từ dữ liệu người dùng nhập vào?(y/n): ")
    if (option == 'y'):
        # GUI
        win = tkr.Tk()
        ...
        tkr.Label(win, text="1.meanfreq:").grid(row=0)
        e1 = Entry(win, width=30)
        e1.grid(row=0, column=1)

        tkr.Label(win, text="2.sd").grid(row=1)
        e2 = Entry(win, width=30)
        e2.grid(row=1, column=1)

        tkr.Label(win, text="3.median").grid(row=2)
        e3 = Entry(win, width=30)
        e3.grid(row=2, column=1)

        tkr.Label(win, text="4.Q25").grid(row=3)
        e4 = Entry(win, width=30)
        e4.grid(row=3, column=1)

        tkr.Label(win, text="5.Q75").grid(row=4)
        e5 = Entry(win, width=30)
        e5.grid(row=4, column=1)

        tkr.Label(win, text="6.IQR").grid(row=5)
        e6 = Entry(win, width=30)
        e6.grid(row=5, column=1)

        tkr.Label(win, text="7.skew").grid(row=6)
        e7 = Entry(win, width=30)
        e7.grid(row=6, column=1)

        tkr.Label(win, text="8.kurt").grid(row=7)
        e8 = Entry(win, width=30)
        e8.grid(row=7, column=1)

        tkr.Label(win, text="9.sp.ent").grid(row=8)
        e9 = Entry(win, width=30)
        e9.grid(row=8, column=1)

        tkr.Label(win, text="10.sfm").grid(row=9)
        e10 = Entry(win, width=30)
        e10.grid(row=9, column=1)

        tkr.Label(win, text="11.mode").grid(row=10)
        e11 = Entry(win, width=30)
        e11.grid(row=10, column=1)

        tkr.Label(win, text="12.centroid").grid(row=11)
        e12 = Entry(win, width=30)
        e12.grid(row=11, column=1)

        tkr.Label(win, text="13.meanfun").grid(row=12)
        e13 = Entry(win, width=30)
        e13.grid(row=12, column=1)

        tkr.Label(win, text="14.minfun").grid(row=13)
        e14 = Entry(win, width=30)
        e14.grid(row=13, column=1)

        tkr.Label(win, text="15.maxfun").grid(row=14)
        e15 = Entry(win, width=30)
        e15.grid(row=14, column=1)

        tkr.Label(win, text="16.meandom").grid(row=15)
        e16 = Entry(win, width=30)
        e16.grid(row=15, column=1)

        tkr.Label(win, text="17.mindom").grid(row=16)
        e17 = Entry(win, width=30)
        e17.grid(row=16, column=1)

        tkr.Label(win, text="18.maxdom").grid(row=17)
        e18 = Entry(win, width=30)
        e18.grid(row=17, column=1)

        tkr.Label(win, text="19.dfrange").grid(row=18)
        e19 = Entry(win, width=30)
        e19.grid(row=18, column=1)
        #
        tkr.Label(win, text="20.modindx").grid(row=19)
        ...
        def randomAndPredict():
            random1()
            predict()
            
        tkr.Button(win, text="Dự đoán", command=predict, activebackground='green',
                   justify='center', bg='green', fg='white').grid(row=25, column=1)
        tkr.Button(win, text="Random", command=random1, activebackground='green',
                   justify='center', bg='green', fg='white').grid(row=24, column=1)
        tkr.Button(win, text="Random + Dự đoán", command=randomAndPredict, activebackground='green',
                   justify='center', bg='green', fg='white').grid(row=26, column=1)
        win.mainloop()
    else:
        os.system("cls")
